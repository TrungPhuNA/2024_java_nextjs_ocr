...
const CertificateForm: React.FC = () => {
	...
	const getData = async (id: any) => {
		setLoading(true);
		const response: any = await COMMON_API.show('certificate', id);
		setLoading(false);
	...
	const submit = async (e: any) => {
		...
		if (id) {
			response = await COMMON_API.update('certificate', id, bodyData);
		} else {
			bodyData.user_id = user?.id;
			response = await COMMON_API.store('certificate', bodyData);
		}
		setLoading(false);
	...
	return (
		<DefaultLayout>
			<Breadcrumb pageName={title} subName="bằng cấp" />
				...
				<div className="flex flex-col gap-5.5 p-6.5">
					<form>
						...
						<div className="flex justify-center mt-5">
							<Link href={'/certificate'} className="inline-flex items-center justify-center 
