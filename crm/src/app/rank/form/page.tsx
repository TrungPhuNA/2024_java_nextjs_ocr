...
const RankForm: React.FC = () => {
	...
	const getData = async (id: any) => {
		setLoading(true);
		const response: any = await COMMON_API.show('rank', id);
		setLoading(false);
	...
	const submit = async (e: any) => {
		...
		if (id) {
			response = await COMMON_API.update('rank', id, bodyData);
		} else {
			bodyData.user_id = user?.id;
			response = await COMMON_API.store('rank', bodyData);
		}
		setLoading(false);
	...
	return (
		<DefaultLayout>
			<Breadcrumb pageName={title} subName="Chức vụ" />
				...
				<div className="flex flex-col gap-5.5 p-6.5">
					<form>
						...
						<div className="flex justify-center mt-5">
							<Link href={'/rank'} className="inline-flex items-center justify-center 
