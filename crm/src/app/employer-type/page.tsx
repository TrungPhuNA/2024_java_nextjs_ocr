...
const OrderList: React.FC = () => {
	...
	const getDataList = async (filters: any) => {
		setLoading(true);
		const response: any = await COMMON_API.getList('employer-type', filters);
		setLoading(false);
	...
	return (
		<DefaultLayout>
			<Breadcrumb subName="Loại nhân viên" pageName="Danh sách" />
			...
			<div className="flex flex-col gap-10">
				<div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
					<div className="px-4 py-6 md:px-6 xl:px-7.5 md:flex md:justify-between w-full">
						...
						<Link href={'/employer-type/form'} className="inline-flex items-center justify-center rounded-md bg-primary px-10 py-2 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10">Create</Link>
					</div>
